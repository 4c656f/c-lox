...
static void errorAtPrevius(const char *message) {
  errorAt(&parser.previus, message);
}
...
static void emitByte(uint8_t byte) {
  writeChunk(getCurrentChunk(), byte, parser.previus.line);
}
...
static void binary() {
  TokenType operator= parser.previus.type;
...
static void parsePrecedence(Precedence precedence) {
  advance();
  ParseFn prefixRule = getRule(parser.previus.type)->prefix;
  if (prefixRule == NULL) {
  ...
  while (precedence <= getRule(parser.current.type)->precedence) {
    advance();
    ParseFn infixRule = getRule(parser.previus.type)->infix;
    infixRule();
  }
}
